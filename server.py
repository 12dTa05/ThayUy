...
class ScanRequest(BaseModel):
    ...
    ports: Optional[str] = Field("1-1000", description="Danh sách các cổng cần quét (vd: '80,443' hoặc '1-1000')")
    output_format: Optional[str] = Field("json", description="Định dạng đầu ra (json, csv, text)")
...
def detect_vulnerabilities(ip, ports=None):
    ...
    try:
        ...
        if ports:
            cmd.extend(['-p', ports])  # Ví dụ: '-p', '80,443'
        cmd.append(ip)
...
class VulnerabilityScanner:
    ...
    def gather_basic_info(self):
        """Thu thập thông tin cơ bản về mục tiêu"""
        logger.info(f"Thu thập thông tin cơ bản về {self.target}")
        
        # Lấy thông tin WHOIS nếu có hostname
    ...
    #     """Phát hiện các công nghệ được sử dụng dựa trên headers và nội dung"""
    #     # Kiểm tra từ server header
    #     if 'Server' in self.headers:
    #         self.technologies.append(self.headers['Server'])
        
    #     # Kiểm tra từ x-powered-by header
    #     if 'X-Powered-By' in self.headers:
    #         self.technologies.append(self.headers['X-Powered-By'])
            
    #     # Kiểm tra từ nội dung HTML
    #     try:
    ...
    #             self.technologies.append(f"CMS: {generator}")
            
    #         # WordPress detection
    #         if soup.select('link[rel="https://api.w.org/"]'):
    #             self.technologies.append("WordPress")
            
    #         # Drupal detection
    #         if "Drupal" in response.text or soup.select('a[href="http://drupal.org"]'):
    #             self.technologies.append("Drupal")
                
    #         # jQuery detection
    #         if "jquery" in response.text.lower():
    #             self.technologies.append("jQuery")
                
    #         # Bootstrap detection
    #         if "bootstrap" in response.text.lower():
    #             self.technologies.append("Bootstrap")
                
    #         # PHP detection
    #         if "PHP" in response.headers.get('X-Powered-By', ''):
    #             self.technologies.append("PHP")
                
    #         # ASP.NET detection
    #         if "ASP.NET" in response.headers.get('X-Powered-By', '') or "ASP.NET" in response.headers.get('X-AspNet-Version', ''):
    #             self.technologies.append("ASP.NET")
    ...
    #             if 'fbcdn.net' in src.lower():  # Dấu hiệu của Facebook
    #                 self.technologies.append('Facebook Framework')
            
    #         self.technologies = list(set(self.technologies))
    #         if self.technologies:
    #             logger.info(f"Công nghệ đã phát hiện: {', '.join(self.technologies)}")
                
    #     except Exception as e:
    #         logger.error(f"Lỗi khi phân tích nội dung web: {str(e)}")
    ...
    def _detect_technologies(self, response=None):  # Giữ tham số response để tương thích
    ...
    def scan_ports(self, ports=None):
        """Quét các cổng mở trên mục tiêu"""
    ...
    def _fallback_port_scan(self, ports):
        """Phương pháp quét cổng thay thế sử dụng socket"""
        found_ports = 0
    ...
    def _check_port(self, port):
        """Kiểm tra một cổng cụ thể có mở không bằng socket"""
        common_services = {
    ...
    def enumerate_services(self):
        """Xác định thông tin chi tiết về các dịch vụ đang chạy"""
    ...
    def _check_common_paths(self, base_url):
        """Kiểm tra các đường dẫn phổ biến trên web server"""
        common_paths = [
            '/robots.txt', 
            '/sitemap.xml', 
            '/admin', 
        ...
        for path in common_paths:
            try:
                ...
                if path_response.status_code == 200:
                    # Kiểm tra nội dung để xác nhận đường dẫn thực sự tồn tại
    ...
    def _check_ftp_service(self, port):
    ...
    def _check_ssh_service(self, port):
    ...
    def _check_smtp_service(self, port):
    ...
    def _check_web_vulnerabilities(self):
        """Kiểm tra các lỗ hổng phổ biến trên ứng dụng web"""
        ...
        for port in web_ports:
            ...
            # Test 1: Kiểm tra XSS phản xạ đơn giản
            self._check_reflected_xss(base_url, port)
            
            # Test 2: Kiểm tra lỗi SQL Injection đơn giản
            self._check_sql_injection(base_url, port)
                
            # Test 3: Kiểm tra thông tin rò rỉ trong HTTP Headers
            self._check_info_disclosure(base_url, port)
                
            # Test 4: Kiểm tra Directory Listing
    ...
    def _check_reflected_xss(self, base_url, port):
        ...
        try:
            ...
            if '<script>alert(1)</script>' in response.text:
                ...
                logger.warning(f"Phát hiện lỗ hổng XSS phản xạ: {base_url}/search")
        except Exception as e:
            logger.debug(f"Kiểm tra XSS không thành công: {str(e)}")
    
    def _check_sql_injection(self, base_url, port):
        ...
        try:
            ...
            if (response.status_code == 200 and normal_response.status_code == 200 and 
                ...
                self.vulnerabilities.append(vuln)
                logger.warning(f"Phát hiện lỗ hổng SQL Injection: {base_url}/item")
            else:
    ...
    def _check_info_disclosure(self, base_url, port):
        """Kiểm tra rò rỉ thông tin trong HTTP Headers"""
        try:
            ...
            if 'X-AspNet-Version' in headers:
                ...
                self.vulnerabilities.append(vuln)
                logger.warning(f"Phát hiện rò rỉ thông tin ASP.NET: {headers['X-AspNet-Version']}")
        except Exception as e:
            logger.debug(f"Kiểm tra rò rỉ thông tin không thành công: {str(e)}")
    
    def _check_directory_listing(self, base_url, port):
            ...
            try:
                ...
                if response.status_code == 200:
                    if 'Index of' in response.text or 'Directory Listing' in response.text:
                        ...
                        self.vulnerabilities.append(vuln)
                        logger.warning(f"Phát hiện Directory Listing: {base_url}{directory}")
            except Exception as e:
                logger.debug(f"Kiểm tra Directory Listing không thành công cho {directory}: {str(e)}")
    
    def query_nvd(self, cpe_string, max_retries=3, retry_delay=2):
        """Truy vấn NVD API để lấy thông tin lỗ hổng với phân trang và retry"""
    ...
    def _process_vulnerabilities(self, vulnerabilities):
        """Xử lý dữ liệu lỗ hổng từ NVD API"""
        results = []
    ...
    def _get_cvss_info(self, cve):
        """Lấy thông tin CVSS severity và score"""
        if not cve:
            return {"severity": "Không xác định", "score": "Không xác định"}
    ...
    def _generate_cpe(self, service_name, version=""):
        """Tạo CPE string từ tên dịch vụ và phiên bản sử dụng thư viện CPE kết hợp với mapping tùy chỉnh"""
        if not service_name:
            return None
    ...
    #     """Phát hiện lỗ hổng thực tế từ NVD với xử lý hiệu quả hơn"""
    #     logger.info("Kiểm tra lỗ hổng thực tế từ NVD...")
        
    #     # Tạo một danh sách các cpe cần truy vấn
    #     cpe_queries = []
        
    #     # Thu thập các CPE từ services
    #     for port, service_info in self.services.items():
    #         service_name = service_info.get('name', '').lower()
    #         version = service_info.get('version', '')
            
    #         cpe = self._generate_cpe(service_name, version)
    #         if cpe:
    #             cpe_queries.append({
    #                 'cpe': cpe,
    #                 'port': port,
    ...
    #     # Thu thập các CPE từ technologies
    #     for tech in self.technologies:
    #         cpe = self._generate_cpe(tech)
    #         if cpe:
    #             cpe_queries.append({
    #                 'cpe': cpe,
    #                 'service': tech,
    ...
    #     # Truy vấn NVD cho tất cả các CPE với giãn cách phù hợp
    #     for query in cpe_queries:
    #         cpe = query['cpe']
            
    #         logger.info(f"Truy vấn NVD cho {query['service']} " + 
    #               (f"v{query['version']} " if 'version' in query and query['version'] else "") +
    #               f"({cpe})")
    ...
    #             # Thêm thông tin khác tùy thuộc vào loại
    #             if query['type'] == 'service':
    #                 vuln_info['port'] = query['port']
    #                 vuln_info['service'] = query['service']
    #                 vuln_info['version'] = query.get('version', '')
    #             else:
    #                 vuln_info['service'] = query['service']
                
    #             self.vulnerabilities.append(vuln_info)
    #             logger.warning(f"Phát hiện: {vuln['id']} ({vuln['severity']}) - {vuln['description']}")
            
    #         # Đảm bảo không vượt quá rate limit
    #         if len(cpe_queries) > 1:
    #             time.sleep(1.5)  # Tăng thời gian chờ để tránh rate limit
    ...
    def detect_vulnerabilities(self):
        ...
        for vuln in vuln_results:
            ...
            self.vulnerabilities.append({
                ...
                'severity': 'Unknown',  # Nmap không cung cấp severity trực tiếp
                'description': vuln['description'],
                'cvss_score': 'N/A',    # Nmap không cung cấp CVSS
                'published': datetime.now().isoformat()
            })

    
    def create_report(self, output_format="json", report_id=None):
        """Lưu báo cáo vào MongoDB với xử lý lỗi toàn diện"""
...
# Dependency để sử dụng MongoDB trong các endpoint
...
async def root():
    """API root endpoint, kiểm tra trạng thái hoạt động của API."""
    return {
...
async def get_statistics(client: MongoClient = Depends(get_mongo_client)):
    """
    Lấy thống kê tổng hợp từ các báo cáo quét
    """
    try:
        db = client[os.environ.get("MONGO_DB", "ThayUy")]
        collection = db[os.environ.get("MONGO_COLLECTION", "scan_reports")]
